...
job_data_copy_id=$(sbatch --parsable \
  ...
  --output="$LOG_OUT" \
  1_copy_from_grid.sbatch ${SOURCE} ${RAW_DESTINATION})
...
job_basecall_id=$(sbatch --parsable \
  ...
  2_dorado_basecall.sbatch ${RAW_DESTINATION} ${PROCESSED_BAM})
...
job_demux_id=$(sbatch --parsable \
  ...
  3_dorado_demux.sbatch ${PROCESSED_BAM} ${DEMUX_DIR})
...
for ((i=0; i<${#BARCODES[@]}; i++)); do
  ...
  job_merge_barcodes_id=$(sbatch --parsable \
    ...
    4_samtools_merge_barcode.sbatch "${SAMPLE_BAM}" "${BARCODE}" "${DEMUX_DIR}")
  ...
  --output="$LOG_OUT" \
    ...
    5_dorado_align.sbatch ${SAMPLE_BAM})
